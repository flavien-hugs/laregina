...
class ProductAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['keywords'].widget.attrs[
            'placeholder'] = 'Ajouter quelques mots-clés (facultatif)'
...
class ProductAttributeValueForm(forms.ModelForm):
    ...
    size = forms.MultipleChoiceField(
        label="Sélectionner les tailles",
        choices=ProductAttributeValue.SIZE_CHOICES
    )
...
ProductCreateFormSet = inlineformset_factory(
    parent_model=Product, model=ProductImage, form=ProductImageForm,
    fields=['product', 'image'], exclude=['timestamp', 'updated'],
    can_delete=False, extra=4, max_num=4
)

ProductAttributeCreateFormset = inlineformset_factory(
    parent_model=Product, model=ProductAttributeValue,
